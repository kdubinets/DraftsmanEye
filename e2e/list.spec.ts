...
test("home page lists drills and switches to curriculum presentation", async ({
  ...
  await expect(
    page.getByRole("heading", { level: 3, name: "Trace Compound Curve" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      level: 3,
  ...
  await expect(page.getByText("New")).toHaveCount(114);
  await expect(page.getByRole("button", { name: "Coming soon" })).toHaveCount(
    0,
  );
  await expect(page.getByRole("button", { name: "Practice" })).toHaveCount(114);
  await expect(
    page
      .getByRole("article")
      .filter({
...
test("home page groups drills and filters by family", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Trace Control 9" }).click();
  ...
  await page.getByRole("button", { name: "All 114" }).click();
...
test("length transfer and angle drills expose family-specific subfilters", async ({
  ...
  await page.getByRole("button", { name: "Angle 24" }).click();
  ...
  await angleCopy
    .locator(".exercise-subfilter-group")
    .filter({ hasText: "Reference" })
    .getByRole("button", { name: "Arbitrary" })
    .click();
